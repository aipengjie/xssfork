...
def help():
    ...
    if args.level < 1 or args.level > 10:
        ...
        logger.error('错误原因 level设置的范围只能在1-10之间')
        exit()
    if args.list is True:
        print_temper()
    elif is_set_url ^ is_set_readfile:
        task_schedule = TaskSchedule(args.url, args.destination, args.level, args.cookie, args.data, args.readfile, args.temper, args.model)
        task_schedule.main()
    else:
        logger = log.get_logger()
        logger.setLevel(logging.DEBUG)
        logger.error('错误原因1:url，readfile至少设置一个')
        logger.error('错误原因2:url，readfile不能同时设置')
        logger.info('查看帮组:python {} -h'.format(__file__))
